...
import com.asura.framework.commons.date.DateCalculator;
...
public class RegExpUtil {
    ...
    //字母数字下划线
    public static final String LETTERS_UNDERLINE = "^[A-Za-z0-9_]+$";
     ...
     * @return
     */
    public static boolean isNameOfLettersOrUnderline(@Nullable String letters) {
        if (Check.isNullOrEmpty(letters)) {
            return false;
        }
        return letters.matches(LETTERS_UNDERLINE);
    }

    public static boolean isMobilePhoneNum(final String mobilePhoneNum) {
        if (!Check.isNullOrEmpty(mobilePhoneNum)) {
            return false;
        }
    ...
    public static boolean idIdentifyCardNum(final String idNum) {
        ...
        if (!DateCalculator.isDate(strYear + "-" + strMonth + "-" + strDay)) {
            return false;
        }
    ...
    public static boolean isChineseCharacter(String character) {
        if (Check.isNullOrEmpty(character)) return false;
    ...
    public static boolean isPassportNum(String passport) {
        if (Check.isNullOrEmpty(passport)) return false;
    ...
    public static boolean isJG(String jg) {
        if (Check.isNullOrEmpty(jg)) return false;
    ...
    public static boolean isHHMMTime(String time) {
        if (Check.isNullOrEmpty(time)) return false;
